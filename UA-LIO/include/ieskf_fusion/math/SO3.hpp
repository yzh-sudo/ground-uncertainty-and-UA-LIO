#ifndef IESKF_FUSION_SO3_HPP_
#define IESKF_FUSION_SO3_HPP_
...
static Eigen::Matrix3d so3Exp(const Eigen::Vector3d& so3) {
  ...
  omega_hat << 0, -so3.z(), so3.y(), so3.z(), 0, -so3.x(), -so3.y(), so3.x(), 0;
  if (so3_norm <= 0.0000001) {
    SO3.setIdentity();
    return SO3 + omega_hat;
  }

  Eigen::Matrix3d so3_skew_sym = skewSymmetric(so3);
  SO3 = Eigen::Matrix3d::Identity() +
        (so3_skew_sym / so3_norm) * sin(so3_norm) +
...
static Eigen::Vector3d SO3Log(const Eigen::Matrix3d& SO3) {
  ...
  Eigen::Vector3d so3(SO3(2, 1) - SO3(1, 2), SO3(0, 2) - SO3(2, 0),
                      SO3(1, 0) - SO3(0, 1));
...
static Eigen::Matrix3d A_T(const Eigen::Vector3d& v) {
  ...
  if (norm < 1e-11) {
    ...
    res = Eigen::Matrix3d::Identity() +
          (1 - std::cos(norm)) / squaredNorm * skewSymmetric(v) +
          (1 - std::sin(norm) / norm) / squaredNorm * skewSymmetric(v) *
              skewSymmetric(v);
  }
  return res;
}
